"""Examination category detection, normalisation and the mandatory match rule."""
from django.test import SimpleTestCase

from exams import exam_categories
    ...
    def test_case_is_not_significant(self):
        for value in ('pilot', 'PILOT', 'Pilot', '  PiLoT  '):
            with self.subTest(value=value):
                self.assertEqual(exam_categories.normalize(value), ExamCategory.PILOT)

    def test_stored_values_and_labels_both_resolve(self):
        self.assertEqual(
            exam_categories.normalize('flight_dispatch'), ExamCategory.FLIGHT_DISPATCH
        )
        self.assertEqual(
            exam_categories.normalize('Flight Dispatch'), ExamCategory.FLIGHT_DISPATCH
        )

    def test_unknown_value_resolves_to_none(self):
        self.assertIsNone(exam_categories.normalize('Air Traffic Control'))
        self.assertIsNone(exam_categories.normalize(''))
        self.assertIsNone(exam_categories.normalize(None))


class DetectionTests(SimpleTestCase):
    def test_detects_each_examination_type(self):
        cases = [
            ('APPLICATION FOR PILOT EXAMINATION', ExamCategory.PILOT),
            ('Application for Cabin Crew Examination', ExamCategory.CABIN_CREW),
            ('APPLICATION FOR AME EXAMINATION', ExamCategory.AME),
            ('Aircraft Maintenance Engineering examination', ExamCategory.AME),
            ('RE: FLIGHT DISPATCH EXAMINATION', ExamCategory.FLIGHT_DISPATCH),
            ('application for flight dispatcher examination', ExamCategory.FLIGHT_DISPATCH),
        ]
        for text, expected in cases:
            with self.subTest(text=text):
                self.assertEqual(exam_categories.detect(text).exam_category, expected)

    def test_flight_dispatch_is_not_mistaken_for_pilot(self):
        """"Flight dispatch" contains no pilot wording; the order must hold."""
        detection = exam_categories.detect('APPLICATION FOR FLIGHT DISPATCH EXAMINATION')
        self.assertEqual(detection.exam_category, ExamCategory.FLIGHT_DISPATCH)
    ...
    def test_lowercase_ame_is_not_treated_as_an_abbreviation(self):
        ...
        detection = exam_categories.detect('we came to submit this application')
        self.assertNotEqual(detection.exam_category, ExamCategory.AME)

    def test_nothing_detected_in_unrelated_text(self):
        detection = exam_categories.detect('Dear Sir, please find our payment attached.')
    ...
    def test_a_passing_mention_does_not_make_it_ambiguous(self):
        ...
        detection = exam_categories.detect(
            'APPLICATION FOR PILOT EXAMINATION. We also list cabin crew staff.'
        )
        self.assertEqual(detection.exam_category, ExamCategory.PILOT)
    ...
    def test_two_examination_contexts_are_flagged_ambiguous(self):
        ...
        self.assertIn(ExamCategory.PILOT, detection.scores)
        self.assertIn(ExamCategory.CABIN_CREW, detection.scores)
...
class MatchRuleTests(SimpleTestCase):
    def test_matching_selections_are_accepted(self):
        for value in ('pilot', 'ame', 'cabin_crew', 'flight_dispatch'):
            with self.subTest(value=value):
                self.assertTrue(exam_categories.matches(value, value))

    def test_case_insensitive_match_is_accepted(self):
        self.assertTrue(exam_categories.matches('pilot', 'PILOT'))
        self.assertTrue(exam_categories.matches('AME', 'ame'))

    def test_different_categories_do_not_match(self):
        self.assertFalse(exam_categories.matches('pilot', 'cabin_crew'))
        self.assertFalse(exam_categories.matches('ame', 'flight_dispatch'))

    def test_missing_detection_does_not_match(self):
        self.assertFalse(exam_categories.matches('pilot', None))
        self.assertFalse(exam_categories.matches('pilot', ''))


class ContextualDetectionTests(SimpleTestCase):
    """The examination category is decided by context, not by keyword presence.
    ...
    def type_of(self, text):
        return exam_categories.detect(text).exam_category
    ...
    def test_cabin_crew_phrasings_all_resolve(self):
        ...
        for text in phrasings:
            with self.subTest(text=text):
                self.assertEqual(self.type_of(text), ExamCategory.CABIN_CREW)
    ...
    def test_examination_context_outranks_a_distractor(self):
        ...
        detection = exam_categories.detect(text)
        self.assertEqual(detection.exam_category, ExamCategory.CABIN_CREW)
        self.assertTrue(detection.contextual)
        self.assertFalse(detection.ambiguous)
    ...
    def test_every_context_template_maps_to_a_canonical_type(self):
        cases = [
            ('REQUEST FOR EXAM DATE FOR PILOT STUDENTS', ExamCategory.PILOT),
            ('APPLICATION FOR AME EXAMINATION', ExamCategory.AME),
            ('AME EXAM', ExamCategory.AME),
            ('Aircraft Maintenance Engineering examination', ExamCategory.AME),
            ('FLIGHT DISPATCH CANDIDATES', ExamCategory.FLIGHT_DISPATCH),
            ('application for flight dispatcher examination', ExamCategory.FLIGHT_DISPATCH),
            ('CPL candidates', ExamCategory.PILOT),
        ]
        for text, expected in cases:
            with self.subTest(text=text):
    ...
    def test_detection_never_invents_a_type(self):
        ...
        allowed = {choice.value for choice in ExamCategory}
        for text in [
            'REQUEST FOR EXAM DATE FOR CABIN CREW AB-INITIO STUDENTS',
            ...
            with self.subTest(text=text):
                result = exam_categories.detect(text).exam_category
...
class TypeRatingDistractorTests(SimpleTestCase):
    ...
    def test_no_aircraft_type_rating_classifies_an_application(self):
        for text in [
            ...
            with self.subTest(text=text):
                self.assertIsNone(exam_categories.detect(text).exam_category)
